...
class AgentProtocol(asyncio.Protocol):
    ...
    def handle_sim_start(self, content: Any) -> None:
        self.static.set_result(content)
    ...
    def handle_request_action(self, content: Any) -> None:
        ...
        self.dynamic.set_result(content)
    ...
    async def move(self, directions: list(DirectionLiteral)) -> AgentProtocol:
        # assert direction in ["n", "s", "e", "w"]
        return await self.send_action("move", directions)
...
class Agent:
    ...
    def adopt(self, role: str) -> Agent:
        ...
        See https://github.com/agentcontest/massim_2022/blob/main/docs/scenario.md#adopt.
        """
        with self._not_shut_down():
            coro = self.protocol.adopt(role)
            future = asyncio.run_coroutine_threadsafe(coro, self.protocol.loop)
        future.result()
        return self
